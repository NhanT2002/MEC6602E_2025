...
parameters::parameters(unsigned int N, double outletBoundaryCondition, double CFL, double epsilon_e, double dx, const std::string& output_filename, double theta)
    : N_(N), outletBoundaryCondition_(outletBoundaryCondition), CFL_(CFL), epsilon_e_(epsilon_e), output_filename_(output_filename), dx_(dx), theta_(theta) {
        Mach_ = 1.25;
        rhoInf_ = 1.0;
        pInf_ = 1.0;
...
void parameters::print() const {
    ...
    std::cout << "epsilon_e: " << epsilon_e_ << std::endl;
