...
def ReadPoints(inputFile, n_points):
    ...
    s_new = np.linspace(0, s_arc[-1], n_points)


    spline=interp1d(s_arc, data[:, 0], kind='cubic')
    x_new = spline(s_new)
    spline=interp1d(s_arc, data[:, 1], kind='cubic')
    y_new = spline(s_new)

    data =np.vstack((x_new, y_new)).T
