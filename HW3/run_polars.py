...
# Configuration
EXE = os.path.join("..", "HW2", "bin", "euler_solver")
TEMPLATE = "input.txt"  # template input file in this directory
OUTDIR = "solver_outputs/crm"
INPUTDIR = "input_cases/crm"
# Angles of attack (degrees) and Mach numbers to sweep. Edit as needed.
ANGLES = np.arange(6.5, 16.0, 0.5)
MACHES = np.array([0.7])
    ...
    for a in ANGLES:
        ...
        for ln in lines:
            ...
            if m:
                ...
                if not ext:
                    ext = ".q"
                new_name = os.path.join(run_dir, f"output_aoa_{aoa_str}{ext}")
            ...
            if m:
                ...
                if not ext:
                    ext = ".txt"
                new_name = os.path.join(run_dir, f"residual_history_aoa_{aoa_str}{ext}")
