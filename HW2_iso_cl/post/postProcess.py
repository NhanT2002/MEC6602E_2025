...
x, y = read_PLOT3D_mesh("../mesh/crm_sharp_128.xyz")
ni, nj, mach, alpha, reyn, time, q_vertex = read_plot3d_2d(f"../output.q")
cp_airfoil, C_L, C_D, C_M = compute_coeff(x, y, q_vertex, mach, alpha, T_inf=300, p_inf=1E5)
plt.figure()
plt.plot(x[0], cp_airfoil)
plt.gca().invert_yaxis()
plt.xlabel("x")
plt.ylabel("Cp")
plt.title("crm_sharp_128 airfoil Cp distribution")
plt.grid()
plt.tight_layout()

Time, R0, R1, R2, R3, cl, cd, cm = read_residual_history(f"../residual_history.txt")
plt.figure()
plt.plot(cl)