...
void macCormack(parameters& params, const std::vector<double>& x, const std::vector<double>& A,
    ...
    std::vector<double> dt(params.N_);
    ...
    // Predictor step
    std::vector<double> Q1_pred(params.N_);
    std::vector<double> Q2_pred(params.N_);
    std::vector<double> Q3_pred(params.N_);
    for (unsigned int i = 1; i < params.N_ - 1; ++i) {
    ...
    // Corrector step
    std::vector<double> Q1_corr(params.N_);
    std::vector<double> Q2_corr(params.N_);
    std::vector<double> Q3_corr(params.N_);
    for (unsigned int i = 1; i < params.N_ - 1; ++i) {
